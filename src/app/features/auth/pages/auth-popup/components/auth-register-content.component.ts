...
@Component({
  ...
  template: `
    <div class="content">
      ...
      <label class="input-wrap" [class.has-error]="passwordTouched && !isPasswordValid()">
...
<path d="M15 2V22" stroke="#367639" stroke-width="1.5" stroke-linecap="round"/>
        ...
        <input [(ngModel)]="password" (ngModelChange)="onPasswordChange()" (blur)="passwordTouched = true" type="password" placeholder="Mật khẩu" />
      </label>
      <p class="error-text" *ngIf="passwordTouched && password && !isPasswordValid()">
        Mật khẩu phải có ít nhất 8 ký tự, gồm chữ hoa, chữ thường và số.
      </p>

      <div class="strength-wrap" *ngIf="password">
        <span class="strength-label">Độ bảo mật: <strong [style.color]="strengthColor">{{ strengthLabel }}</strong></span>
        <div class="bars">
          @for (bar of [1,2,3,4]; track bar) {
            <span class="bar" [class.active]="strengthScore >= bar" [style.background]="strengthScore >= bar ? strengthColor : '#bfd0e2'"></span>
          }
        </div>
      </div>
      ...
      <div class="rules" *ngIf="password">
        <div class="rule" [class.met]="password.length >= 8">
          <span class="rule-icon">{{ password.length >= 8 ? '✓' : '✗' }}</span> Ít nhất 8 ký tự
        </div>
        <div class="rule" [class.met]="hasUpperCase()">
          <span class="rule-icon">{{ hasUpperCase() ? '✓' : '✗' }}</span> Có chữ hoa (A-Z)
        </div>
        <div class="rule" [class.met]="hasLowerCase()">
          <span class="rule-icon">{{ hasLowerCase() ? '✓' : '✗' }}</span> Có chữ thường (a-z)
        </div>
        <div class="rule" [class.met]="hasNumber()">
          <span class="rule-icon">{{ hasNumber() ? '✓' : '✗' }}</span> Có chữ số (0-9)
        </div>
        <div class="rule" [class.met]="hasSpecialChar()">
          <span class="rule-icon">{{ hasSpecialChar() ? '✓' : '✗' }}</span> Có ký tự đặc biệt (!&#64;#$...)
        </div>
      </div>

      <label class="input-wrap" [class.has-error]="confirmTouched && confirmPassword && !isConfirmMatch()">
        <span class="icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M12 4H10C6.229 4 4.343 4 3.172 5.172C2.001 6.344 2 8.229 2 12C2 15.771 2 17.657 3.172 18.828C4.344 19.999 6.229 20 10 20H12M15 4C18.114 4.01 19.765 4.108 20.828 5.172C22 6.343 22 8.229 22 12C22 15.771 22 17.657 20.828 18.828C19.765 19.892 18.114 19.99 15 20" stroke="#367639" stroke-width="1.5" stroke-linecap="round"/>
<path d="M9 12C9 12.2652 8.89464 12.5196 8.70711 12.7071C8.51957 12.8946 8.26522 13 8 13C7.73478 13 7.48043 12.8946 7.29289 12.7071C7.10536 12.5196 7 12.2652 7 12C7 11.7348 7.10536 11.4804 7.29289 11.2929C7.48043 11.1054 7.73478 11 8 11C8.26522 11 8.51957 11.1054 8.70711 11.2929C8.89464 11.4804 9 11.7348 9 12ZM13 12C13 12.2652 12.8946 12.5196 12.7071 12.7071C12.5196 12.8946 12.2652 13 12 13C11.7348 13 11.4804 12.8946 11.2929 12.7071C11.1054 12.5196 11 12.2652 11 12C11 11.7348 11.1054 11.4804 11.2929 11.2929C11.4804 11.1054 11.7348 11 12 11C12.2652 11 12.5196 11.1054 12.7071 11.2929C12.8946 11.4804 13 11.7348 13 12Z" fill="#367639"/>
<path d="M15 2V22" stroke="#367639" stroke-width="1.5" stroke-linecap="round"/>
        ...
        <input [(ngModel)]="confirmPassword" (blur)="confirmTouched = true" type="password" (keydown.enter)="onEnter()" placeholder="Xác nhận mật khẩu" />
      </label>
      <p class="error-text" *ngIf="confirmTouched && confirmPassword && !isConfirmMatch()">
...
export class AuthRegisterContentComponent {
  ...
  hasSpecialChar(): boolean { return /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(this.password); }

  isPasswordValid(): boolean {
    return this.password.length >= 8 && this.hasUpperCase() && this.hasLowerCase() && this.hasNumber();
  }
  ...
  isValidForm(): boolean {
    return this.isPasswordValid() && this.isConfirmMatch() && this.confirmPassword.length > 0;
  }
